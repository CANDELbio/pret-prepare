...
(ns org.parkerici.pret.import.engine
  (:require [clojure.data.csv :as csv]
            ...
            [org.parkerici.pret.db.metamodel :as metamodel]
            [org.parkerici.pret.matrix :as matrix]
            [org.parkerici.pret.import.engine.parse.config :as parse.config]
            [org.parkerici.pret.import.engine.parse.mapping :as parse.mapping]
            [org.parkerici.pret.import.engine.parse.matrix :as parse.matrix]
            [org.parkerici.pret.import.engine.parse.data :as parse.data]
            [org.parkerici.pret.util.io :as io]
            [org.parkerici.pret.util.text :as text]
            [org.parkerici.pret.util.uuid :as uuid]
            [org.parkerici.pret.db.schema :as schema]
            [org.parkerici.pret.util.collection :as coll]))
...
(defn threads-per-file []
  (or (Long/getLong "org.parkerici.pret.prepare.threads")
  ...
  (with-open [rdr (jio/reader in-f)
    ...
    (let [out-ch (a/chan 10000)
          ...
          xf (comp->anomalies
               ...
               (remove #(= % :pret/omit))
               ;; TODO: rewire back in when validations are data-driven and can be generically applied.
               #_(map record/validate))]
...
(defn run-matrix-jobs!
  ...
  [target-dir full-import-ctx matrix-jobs]
...
(defn create-entity-data
  ...
  (try
    (let [start (System/currentTimeMillis)
          ...
          matrix-results (run-matrix-jobs! target-dir full-import-ctx matrix-jobs)
